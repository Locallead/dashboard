...
import { addServicePage } from "@/app/actions";
...
export default function ServicePageForm({
  ...
  async function onSubmit(data: FormValues) {
    ...
    try {
      const result = await addServicePage({ id, data });
  ...
  const generateServicePageContent = async () => {
    setIsGenerating(true);
    const response = await fetch("/api/service-page", {
      method: "POST",
      body: JSON.stringify({
  ...
  console.log(businessName, address, state, city);

  return (
    <div className="py-6">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-3xl font-bold mb-6">Service Page Editor</h1>
        <Button onClick={generateServicePageContent}>
          {isGenerating ? (
            <>
            ...
            <TabsContent value="hero">
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Configure the main hero section of your service page.
                  </CardDescription>
                </CardHeader>
