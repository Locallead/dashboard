...
export async function POST(req: Request) {
  ...
  try {
    const { object } = await generateObject({
      ...
      prompt: `You are a SEO Expert. And you create websites optimized for local SEO, making them seo optimzied and hyper-localized. You are given the Business name. This is a garage door services provider business. The content you produce is gonna be duplicated in multiple location pages so replace every use of location & state uses with {location}. Business name is ${business} try to use as many keywords as possible in the content.`,
    });
